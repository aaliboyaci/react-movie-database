...
export async function fetchMovieTrailers(movieId: string | undefined): Promise<MovieTrailers[]> {
    try {
        const movieTrailersResponse = await getMovieTrailers(movieId);
        const movieTrailers: MovieTrailers[] = movieTrailersResponse?.map((trailer: MovieTrailersDTO) => {
