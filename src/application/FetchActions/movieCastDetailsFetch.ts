...
import { CrewMember, CastMember, MovieCastDetails } from '../Types/MovieCastDetailsTypes';


export async function fetchMovieCastDetails(showId: string | undefined): Promise<MovieCastDetails> {
    try {
        const movieCastDetailsResponse = await getMovieCastDetails(showId);
        const director: CrewMember | undefined = movieCastDetailsResponse.crew?.find(
            (person: CrewMember) => person.job === 'Director' && person.department === 'Directing'
        );

        const mainCast: CastMember[] | undefined = movieCastDetailsResponse.cast?.slice(0, 3);
