import { getMovieDetails, getPersonDetails, getPersonPopMovies } from '../../data-access/apiAccess';
...
export async function fetchPersonDetails(personID: string | undefined): Promise<{ personDetails: PersonDetailsTypes; popMovies: PersonPopMoviesTypes[]; }> {
    try {
        ...
        const cast:PersonPopMoviesTypes[] |undefined= personPopMoviesResponse.cast;
        const crew:PersonPopMoviesTypes[] |undefined= personPopMoviesResponse.crew;
        ...
        ///
            ...
            if (personDetails.knownForDepartment === "Acting") {
                const filteredCast = cast?.filter((item: any) => !item.character.includes('(voice)'));
            ...
            if (personDetails.knownForDepartment === "Directing") {
                const filteredCast = crew?.filter((item: any) => item.department.includes("Directing"));
                const sortedCast = filteredCast?.sort((a: any, b: any) => b.popularity - a.popularity);
                const top5Films = sortedCast?.slice(0, 5);
                popMovies=  top5Films || [];   
            }
...
export function usePersonDetailsFetch(personID: string | undefined) {
    return useFetch<{ personDetails: PersonDetailsTypes; popMovies: PersonPopMoviesTypes[]; }>(() => fetchPersonDetails(personID));
}
